...
SELECT * FROM DEPT;
--answer
SELECT E.ENAME,(SELECT ENAME FROM EMP WHERE E.MGR = EMPNO ) AS MANAGER
 FROM EMP E
...
SELECT GRADE, ENAME AS "등급별가장작은급여"
 FROM EMP E
  JOIN SALGRADE SG ON SG.LOSAL <= E.SAL
   AND E.SAL <= SG.HISAL
  WHERE E.SAL IN (SELECT E.ENAME FROM EMP WHERE SAL = MIN(SAL))
  GROUP BY GRADE, ENAME
 ;
