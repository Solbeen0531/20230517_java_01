...
//		contentPane.add(txtName); //component를 contentPane에 추가
		...
		btnSave.addActionListener(new MyNameActionListener());
	...
	class MyNameActionListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (source instanceof JTextField) {
				System.out.println("JTextField에서 enter 눌렀네요");
				if (((JButton) source).getText().trim().length() == 0) {
					((JButton) source).setText("이름을 입력해주세요");
				} else {
					((JButton) source).setText(((JTextField) source).getText());
				}
